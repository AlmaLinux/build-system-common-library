...
class BaseSourceDownloader:
    ...
    def iter_source_records(self):
        ...
        for line in open(metadata_file, 'r').readlines():
            ...
            final_path = (
                os.path.join(self._sources_dir, path) if path else None
            )
    ...
    def download_all(self) -> bool:
        ...
        for checksum, path in self.iter_source_records():
            ...
            try:
                ...
                logging.exception(
                    'Cannot download %s with checksum %s', path, checksum
                )
...
class CentpkgDowloader(BaseSourceDownloader):
    ...
    def download_all(self):
        ...
        code, out, err = (
            local['centpkg']
            .with_cwd(self._sources_dir)
            .run(args=('sources', '--force'), retcode=None)
        )
        if code != 0:
            return False
        return True
